...
def transform(X_original, variance):
    ...
    red_converted, red_req_dimension, red_original_dim = perform_transform(
        red, variance
    )
    green_converted, green_req_dimension, green_original_dim = perform_transform(
        green, variance
    )
    blue_converted, blue_req_dimension, blue_original_dim = perform_transform(
        blue, variance
    )
    ...
    X_new = (
        np.dstack((red_converted, green_converted, blue_converted)) * 255.999
    ).astype(np.uint8)
