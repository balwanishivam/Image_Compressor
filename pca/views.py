...
class SaveFileView(views.APIView):
    def post(self, request, *args, **kwargs):
        ...
        if request_file:
            ...
            file_converted=save(X_reduced,"jpg",file_name)
            new_file_url=fs.url(file_converted)
            return response.Response(
                {"success": True, "url": fileurl,"req_dim":req_dim,"orig_dim":orig_dim,"compressed_file":new_file_url}, status=status.HTTP_200_OK
            )
        else:
            return response.Response(
